...
from .encoding_base import Clause, Encoding, EncodingException
import itertools
...
################################################################################
    ...
    def make_variables(self, horizon: int) -> None:
        """ Make the variables (state and action fluents) for the problem.
            ...
            NOTE#! self.problem.actions and self.problem.propositions.

            NOTE#! Use str(proposition) to get name of a proposition and str(action)
            ...
            So, NOTE#! self.new_cnf_code(4, str(a), a) will create a new variable and
        ...
        for action in self.problem.actions:
            for s in range(0, horizon):
                a_int = self.new_cnf_code(s, str(action), action)
                self.action_fluent_codes[(action, s)] = a_int
                
        for pro in self.problem.propositions:
    ...
    def make_initial_state_and_goal_axioms(self, horizon: int) -> None:
        """ Make clauses representing the initial state and goal.
            ...
            initial state of the problem holds at step 0. 
            NOTE#? The Propositions which must be true are in self.problem.pos_initial_state and the
            NOTE#? Propositions which must be false are in self.problem.neg_initial_state.
            ...
            NOTE#! A clause is a list of positive or negative integers (representing
            ...
            NOTE#! The type of the start state clauses should be "start" and the type
            #! of the goal clauses should be "goal".
        """

        for pos_p in self.problem.pos_initial_state:
            pos_P_int = self.proposition_fluent_codes[(pos_p, 0)]
            self.add_clause([pos_P_int], "start")
            
        for neg_p in self.problem.neg_initial_state:
            neg_p_int = self.proposition_fluent_codes[(neg_p, 0)]
            self.add_clause([neg_p_int * -1], "start")
            
        for goal_p in self.problem.goal:
            goal_p_int = self.proposition_fluent_codes[(goal_p, horizon)]
            self.add_clause([goal_p_int], "goal")
    ...
    def make_precondition_and_effect_axioms(self, horizon: int) -> None:
        """ Make clauses representing action preconditions and effects.
            ...
            NOTE#! In your clauses use the variables from self.action_fluent_codes and
            #! self.proposition_fluent_codes.

            NOTE#! Precondition clauses have the type 'pre' and effect clauses have the
            #! type "eff".
        ...
        for act_s, a_int in self.action_fluent_codes.items():
            action, step = act_s
            pre_conds    = action.preconditions
            pos_effs     = action.pos_effects
            neg_effs     = action.neg_effects
            if len(set(pos_effs).intersection(set(neg_effs))) != 0:
                continue
            
            for pre in pre_conds:
                self.add_clause([-1 * a_int, self.proposition_fluent_codes[(pre, step)]], "pre")
            for pos_e in pos_effs:
                self.add_clause([-1 * a_int, self.proposition_fluent_codes[(pos_e, step + 1)]], "eff")
    ...
    def make_explanatory_frame_axioms(self, horizon: int) -> None:
        ...
        for pro_s, p_int in self.proposition_fluent_codes.items():
            prop, step = pro_s
            if step == horizon:
                continue
            next_p_int = self.proposition_fluent_codes[(prop, step + 1)]
            pos_acts   = prop.pos_effects
            neg_acts   = prop.neg_effects
            
            clause_pos = [p_int, -1 * next_p_int]
            for act in pos_acts:
                clause_pos.append(self.action_fluent_codes[(act, step)])
            clause_neg = [-1 * p_int, next_p_int]
            for act in neg_acts:
                clause_neg.append(self.action_fluent_codes[(act, step)])
                
            self.add_clause(clause_pos, "frame")
            self.add_clause(clause_neg, "frame")
    ...
    def make_interference_mutex_axioms(self, horizon: int) -> None:
        """ Make clauses preventing interfering actions from being executed in parallel.
            ...
            attributes stored in #!Proposition objects,
            instead of the `neg_effects` and `preconditions` attributes in #?Action objects.
        ...
        for prop in self.problem.propositions:
            acts_pre  = set(prop.preconditions)
            acts_neg  = set(prop.neg_effects)
            acts_pos  = set(prop.pos_effects)
        ...
        for a1, a2 in clauses_act:
            if (a1, a2) in clauses_delete or (a2, a1) in clauses_delete or a1 == a2:
                continue
            else:
                for i in range(0, horizon):
                    a1_int = self.action_fluent_codes[(a1, i)]
